...
def build_server_cmd(args: argparse.Namespace, ncmoe: int) -> List[str]:
    ...
    instruct = "deepseek2.expert_used_count=int:" + str(ncmoe)
    cmd = [
        args.server_bin,
        "-m",
        args.model,
...
def evaluate_squad_for_current_server(
    ...
    for i in tqdm(range(num_samples), desc="SQuAD eval", ncols=100):
        ...
        try:
            predict, latency = post_completion_with_retry(
                completion_url,
                payload,
            ...
            predict = ""
            failed.append({"index": i, "id": question_id, "error": str(exc)})

        references.append({"answers": answers, "id": question_id})
        predictions.append({"prediction_text": predict.strip(), "id": question_id})
    ...
    for ncmoe in range(args.ncmoe_start, args.ncmoe_end + 1):
        ...
        try:
            ...
            print(
                ...
                f"F1={row['f1']:.4f}, "
                f"avg_latency_s={row['avg_latency_s']}"
            )
