...
# 直接读取 AlpacaEval 官方 JSON 文件，避免触发 alpaca_eval.py dataset script
ALPACA_EVAL_URL = (
...
BATCH_SIZE = 1000
SAVE_PATH = "alpaca_eval_token_length_distribution.png"

# 统计方式：
# "instruction"：只统计用户指令长度，推荐用于助手类应用输入长度分布
# "instruction_output"：统计 instruction + reference output 的总长度
TEXT_FIELD = "instruction"


# =========================
# 加载 AlpacaEval 数据和 tokenizer
# =========================
...
for example in dataset:
    if TEXT_FIELD == "instruction":
        text = example["instruction"]

    elif TEXT_FIELD == "instruction_output":
        text = (
            "Instruction:\n"
            + example["instruction"]
            + "\n\nReference output:\n"
        ...
        raise ValueError("TEXT_FIELD must be one of: instruction, instruction_output")
...
plt.ylabel("Count")
plt.title(f"AlpacaEval Token Length Distribution ({TEXT_FIELD})")
